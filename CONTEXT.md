...
- **Cron**: `.github/workflows/sync-timbo.yml` — cada 6h UTC (`17 */6 * * *`) + `workflow_dispatch`, llama al server con secrets `TIMBO_API_URL` y `TIMBO_SYNC_TOKEN`. Falta: configurar los secrets en el repo y setear `TIMBO_SYNC_TOKEN` en Vercel.

        ...
        <>
          ...
          <div className="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3">
            <div className="card p-4">
              ...
              <p className="mt-1.5 font-display text-2xl font-bold text-primary-light tabular-nums">{formatPesos(presup.resultado.ingreso)}</p>
            ...
            <div className="card p-4">
              ...
              <p className="mt-1.5 font-display text-2xl font-bold text-primary-light tabular-nums">{formatPesos(presup.recaudado)}</p>
              <p className="mt-1 text-xs text-white/60 leading-relaxed">
            ...
            <div className={cn('card p-4', presup.resultado.balance >= 0 ? 'border-primary/30 bg-primary/[0.04]' : 'border-red-500/30 bg-red-500/[0.04]')}>
              <p className="text-[10px] font-mono uppercase tracking-wider text-white/50">Balance</p>
              <p className={cn('mt-1.5 font-display text-2xl font-bold tabular-nums', presup.resultado.balance >= 0 ? 'text-primary-light' : 'text-red-400')}>
                {formatPesos(presup.resultado.balance)}
              </p>
              <p className="mt-1 text-xs">
                <span className={presup.resultado.balance >= 0 ? "text-primary-light/80" : "text-red-400/80"}>
            ...
            <div className="card p-4">
              ...
              {presup.recaudado != null ? (
                <>
                  <p className={cn('mt-1.5 font-display text-2xl font-bold tabular-nums', presup.recaudado - presup.resultado.gastos >= 0 ? 'text-primary-light' : 'text-red-400')}>
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div className="card p-4">
                      ...
                      <p className="mt-1 text-2xl font-bold text-primary-light">{formatPesos(totalData.totales.ingreso)}</p>
                    </div>
                    <div className="card p-4">
                  ...
                  {totalData.totales.recaudado != null && (
                    <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-3">
                      <div className="card p-4">
                        ...
                        <p className="mt-1 text-2xl font-bold text-primary-light">{formatPesos(totalData.totales.recaudado)}</p>
                      ...
                      <div className="card p-4">
                        ...
                        <p className={cn('mt-1 text-2xl font-bold', totalData.totales.balance >= 0 ? 'text-primary-light' : 'text-red-400')}>
                          {formatPesos(totalData.totales.balance)}
                        </p>
                      </div>
                      <div className="card p-4">
                        <p className="text-xs text-white/50">Balance real</p>
                        <p className={cn('mt-1 text-2xl font-bold', totalData.totales.recaudado - totalData.totales.gastos >= 0 ? 'text-primary-light' : 'text-red-400')}>
                  ...
                  <div className={cn('mt-3 card p-4', totalData.totales.balance >= 0 ? 'border-primary/30 bg-primary/5' : 'border-red-500/30 bg-red-500/5')}>
                  ...
                  {totalData.gym && (
                    <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-3">
                      ...
                      <div className="card p-4">
                        ...
                        <p className="mt-1.5 text-2xl font-bold text-primary-light tabular-nums">{formatPesos(totalData.gym.recaudado)}</p>
                        <p className="mt-1 text-xs text-white/60">lo que pagaron los que van</p>
                      </div>
                      <div className={cn('card p-4', totalData.gym.faltaCobrar > 0 ? 'border-amber-500/30 bg-amber-500/5' : 'border-primary/30 bg-primary/5')}>
                        <p className="text-[10px] font-mono uppercase tracking-wider text-white/50">Gym · falta cobrar</p>
                        <p className={cn('mt-1.5 text-2xl font-bold tabular-nums', totalData.gym.faltaCobrar > 0 ? 'text-amber-300' : 'text-primary-light')}>
                        ...
                        {totalData.porEquipo.map((e) => {
                          ...
                          return (
                            <tr key={e.teamId} className={cn('border-t border-outline/60', e.balance < 0 ? 'bg-red-500/5' : '')}>
                              ...
                              <td className={cn('p-3 font-semibold', e.balance >= 0 ? 'text-primary-light' : 'text-red-400')}>
                          ...
                          {totalData.porMes.map((m) => {
                            ...
                            return (
                              <tr key={m.mes} className="border-t border-outline/60">
                                ...
                                <td className="p-3 text-primary-light">{formatPesos(m.recaudado)}</td>
                                <td className="p-3 text-white/70">{m.estimado > 0 ? pct + "%" : "—"}</td>
                                <td className={cn('p-3 font-semibold', m.acumulado > 0 ? 'text-primary-light' : 'text-white/40')}>

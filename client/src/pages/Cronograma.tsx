...
const LUGARES = [
  { name: "Polideportivo", color: "border-primary/40 bg-primary/15 text-primary-light", emoji: "🏟️" },
              ...
              <div>
                ...
                {equiposOrdenados.length > 0 && (
                  <div className="mb-6 flex flex-wrap items-center gap-3">
                    <label htmlFor="filtro-categoria" className="text-sm text-white/70 font-medium">Filtrar por categoría:</label>
                    <select
                      id="filtro-categoria"
                      aria-label="Filtrar por categoría"
                      value={categoriaFiltro}
                      onChange={(e) => setCategoriaFiltro(e.target.value)}
                      className="min-h-[40px] px-4 py-2.5 rounded-lg bg-surface-1 border border-outline text-sm text-white focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/30 transition-colors"
                    >
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  {schedule.semana.map((d) => {
                    ...
                    const partidosFiltrados =
                      categoriaFiltro === "todas"
                        ? d.partidos
                        : d.partidos.filter((p) => p.team.name === categoriaFiltro);

                    const tieneActividad = bloquesFiltrados.length > 0 || partidosFiltrados.length > 0;
                    ...
                    return (
                      <div
                        key={d.fecha}
                        className="rounded-lg border border-outline bg-surface overflow-hidden border-l-2 border-l-primary/40"
                      >
      ...
      return (
        <div key={b.id} className={cn('rounded-md border px-2.5 py-2', info.color)}>
          <div className="flex items-center justify-between gap-1">
            <p className="font-mono text-xs font-semibold tabular-nums flex items-center gap-1 min-w-0">
          ...
          <p className="text-sm font-semibold mt-0.5 truncate">{b.team?.name ?? "Actividad libre"}</p>
          <p className="text-[11px] opacity-80 truncate">{b.place}</p>

...
export default function Home() {
  ...
  return (
    <div className="min-h-screen">
      {/* Header */}
      <header className="bg-gradient-to-b from-blue-800 to-blue-900 px-6 py-8">
        <div className="max-w-4xl mx-auto">
          <p className="text-accent font-mono text-xs uppercase tracking-widest">Club José Hernández</p>
          <h1 className="font-display text-3xl md:text-4xl font-bold mt-1">
            Futsal de la ciudad
          </h1>
          <div className="flex gap-3 mt-5 text-sm">
            <Link to="/mi-cuota" className="bg-accent text-black px-4 py-2 rounded-lg font-semibold hover:bg-yellow-400 transition">
              Consultar mi cuota
            </Link>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        <h2 className="font-display text-2xl font-bold flex items-center gap-2">
          <span className="inline-block w-2 h-6 bg-accent rounded" />
          Próximos partidos
        </h2>
          ...
          {matches.map((m) => (
            <article key={m.id} className="rounded-xl bg-white/5 border border-white/10 p-5 flex flex-col md:flex-row md:items-center gap-4">
              <div className="md:w-48">
                <p className="text-accent font-mono text-xs uppercase">{formatFecha(m.dateTime)}</p>

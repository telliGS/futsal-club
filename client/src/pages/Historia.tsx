...
export default function Historia() {
  ...
  return (
    <Layout>
      ...
      <div className="relative bg-gradient-to-b from-primary-dark to-surface overflow-hidden border-b border-outline bg-pitch">
        <div className="relative max-w-5xl mx-auto px-6 py-16 md:py-20 text-center">
          <h1 className="font-display font-bold text-fluid-4xl">
            Nuestra <span className="text-primary-light">historia</span>
          </h1>
          <p className="mt-4 text-white/80 text-fluid-base max-w-2xl mx-auto leading-relaxed">
            Conocé el camino del Club José Hernández: desde sus orígenes en el barrio
            hasta los títulos que nos llenan de orgullo.
          </p>
          <div className="mt-6 flex justify-center gap-4 text-fluid-sm text-white/50 flex-wrap">
            <span>🏠 2010</span>
            <span>·</span>
            <span>🏆 2016</span>
            <span>·</span>
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 md:py-16">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          <div>
            ...
            <div className="mt-4 space-y-3 text-white/70 leading-relaxed">
              <p className="text-fluid-base text-white/80">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <Link
                to="/"
                className="btn bg-surface-2 border border-outline text-white hover:border-primary/40 hover:bg-surface-1 inline-flex items-center gap-2 touch-target"
              >
                ← Volver al inicio
              </Link>
            </div>
        ...
        <section className="mt-16">
          ...
          <div className="mt-8 space-y-6">
            {hitos.map((hito, index) => (
              <div
                key={index}
                className="flex items-start gap-4 sm:gap-6 border-l-2 sm:border-l-3 border-primary/40 pl-4 sm:pl-6 pb-6 last:pb-0"
              >
                <span className="shrink-0 text-2xl sm:text-3xl">{hito.icon}</span>
                <div>
                  <span className="font-display font-bold text-fluid-xl text-primary-light tabular-nums block">
                    {hito.año}
                  </span>
                  <p className="text-white/70 text-fluid-sm">{hito.descripcion}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="mt-16">
          ...
          <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div className="aspect-square rounded-xl overflow-hidden border border-outline bg-surface-1 hover:border-primary/40 transition-colors">
              <img src="/images/historia-1.jpg" alt="Historia del club 1" className="w-full h-full object-cover" loading="lazy" decoding="async" />
            </div>
            <div className="aspect-square rounded-xl overflow-hidden border border-outline bg-surface-1 hover:border-primary/40 transition-colors">
              <img src="/images/historia-2.jpg" alt="Historia del club 2" className="w-full h-full object-cover" loading="lazy" decoding="async" />
            </div>
            {[3, 4, 5, 6].map((i) => (
              <div
                key={i}
        ...
        <section className="mt-16">
          ...
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {testimonios.map((t, index) => (
              <div
                key={index}
                className="card p-5 sm:p-6 animate-fade-up touch-target"
        ...
        <section className="mt-16 border-t border-outline pt-12 text-center">
          ...
          <div className="mt-4 flex justify-center gap-6 text-2xl sm:text-3xl">
            <a
              href="https://www.instagram.com/josehernandezfs/"
              target="_blank"
              rel="noopener noreferrer"
              className="touch-target text-white/60 hover:text-primary-light transition-colors"
